...
public class JedisUtil  {
    ...
    private static JedisPool getPool(String ip,int port) {
        ...
        if(!maps.containsKey(key)) {
            ...
            //如果pool中没有链接, 则需要等待的时间(ms),默认不等待
//            config.setMaxWaitMillis(configuration.getMaxwait()); //commons-pool-2.0中的参数
//            config.setMaxWait(configuration.getMaxwait()); // commons-pool-1.5.5
            ...
            try{  
                 ...
                 */
                pool = new JedisPool(config, ip, port,configuration.getTimeout());
                maps.put(key, pool);
            } catch(Exception e) {
            	logger.debug("Init JedisPoll failed! " + e.getMessage());
            }
        }else{
        	pool = maps.get(key);
        }
	...
	public Jedis getJedis() {
		...
		do{
    		try{ 
    			jedis = getPool(configuration.getIp(),configuration.getPort()).getResource();
    			//log.info("get redis master1!");
    		} catch (Exception e) {
    			logger.debug("get redis master1 failed! " + e.getMessage());
    			 // 销毁对象  
    			getPool(configuration.getIp(),configuration.getPort()).returnBrokenResource(jedis);  
    		}
    		count++;
		}while(jedis==null&&count<configuration.getRetryNum());
		return jedis;
	}
	...
	public void closeJedis(Jedis jedis) {
		if(jedis != null) {
		    getPool(configuration.getIp(),configuration.getPort()).returnResource(jedis);
		}
	}
}
