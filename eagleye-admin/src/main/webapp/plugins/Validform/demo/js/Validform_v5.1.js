...
(function($,win,undef){
	...
	Validform.defaults={
	     //	tiptype:1,系统默认的，如果要使用系统默认的，请将该注释打开
	     //=======修改的tip样式及触发方法==start=====================================================================
		tiptype:function(msg,o,cssctl){
			if(!o.obj.is("form")){//验证表单元素时o.obj为该表单元素，全部验证通过提交表单时o.obj为该表单对象;
				var objtip=o.obj.parent("div").find(".Validform_checktip");
				//alert(msg);
				var my_html='<div class="info"><span class="Validform_checktip Validform_wrong">'+msg+'</span><span class="dec"><s class="dec1">&#9670;</s><s class="dec2">&#9670;</s></span></div>';
				cssctl(objtip,o.type);
				objtip.text(msg);
				if(objtip.length == 0){
					o.obj.parent("div").append(my_html);
				};
				
				var infoObj=o.obj.parent("div").find(".info");
				if(o.type==2){
					infoObj.fadeOut(200);
				}else{
	...
	Validform.util={
		dataType:{
			...
			"e":function(gets,obj,curform,regxp){
					...
					if(reg1.test(gets)&&gets.length<40&&gets.length>6){return true;}
					else return false;
					//注意return可以返回true 或 false 或 字符串文字，true表示验证通过，返回字符串表示验证失败，字符串作为错误提示显示，返回false则用errmsg或默认的错误提示;
			},
			"url":/^(\w+:\/\/)?\w+(\.\w+)+.*$/,
			"range" : function(gets,obj,curform,regxp){
				...
				var range=$(obj).attr("datatype").substr("range".length);
				var split=range.split("-");					
				return parseInt(split[0])<=parseInt(gets)&&parseInt(split[1])>=parseInt(gets);
			},
			"phone":/^(\(\d{3,4}\)|\d{3,4}-|\s)?\d{8}$/
		},
		...
		regcheck:function(datatype,gets,obj){
			...
			*/
				...
				for(var name in Validform.util.dataType){
					if(datatype.indexOf(name)!=-1&&Validform.util.toString.call(Validform.util.dataType[name])=="[object Function]"){
						matchDataType=Validform.util.dataType[name];
					}	
				}
			}else{
				matchDataType=Validform.util.dataType[datatype];
			} 
			if(Validform.util.toString.call(matchDataType)=="[object Function]"){
				passed=matchDataType(gets,obj,curform,Validform.util.dataType);
				if(passed === true){
					type=2;
