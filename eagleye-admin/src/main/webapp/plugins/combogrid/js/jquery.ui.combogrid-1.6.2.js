...
$.widget( "cg.combogrid", {
	options: {
		resetButton: false,
		searchReset:true,//查询的时候默认将需要清空的数据清空
		resetFields: null,
		searchButton: false,
		searchIcon:false,
		okIcon:false,
	...
	_create: function() {
		var self = this,
			...
			suppressKeyPress;
			self.options.resetButton=$.isFunction(self.options.resetButton)?self.options.resetButton(self.element):self.options.resetButton;
			self.options.resetFields=$.isFunction(self.options.resetFields)?self.options.resetFields(self.element):self.options.resetFields;
			self.options.colModel=$.isFunction(self.options.colModel)?self.options.colModel(self.element):self.options.colModel;
			...
			//为 搜索框添加双击事件
			self.element.bind("dblclick.combogrid",function(){
					self.element.trigger('focus.combogrid');
					self._search(self.element.val());
					self.element.trigger('focus.combogrid');
					self.element.trigger("search");
			...
			self.element.bind("search.combogrid",function(){
					if(self.options.searchReset&&self.options.resetFields!=null){
						$.each(self.options.resetFields, function() {
						$( ''+this ).val('');
				 		});
					}
		...
		//ESCAPE needs this workaround
			...
			this.options.source =  function( request, response ) {
				var url=self.options.url;
				 url= $.isFunction(url)?url(self.element):url;
				 if(!url)return;
				$.ajax({
					url: url,
					type:"POST",
					...
					data: {
						iSortCol:0,
						mDataProp: self.options.sidx,
						pageNow : self.page,
						sSortDir: self.options.sord,
						pageSize: self.options.rows,
					...
					success: function( data ) {
						...
						/**
							...
							*/
							response(data.records, data.total, $.map( data.result, function( item ) {
								return item;
							}));
	...
	close: function( event ) {
		...
		if ( this.menucombo.element.is(":visible") ) {
			...
			this.cssCol="";
			this.lastOrdered="";
			this.options.rows=self.options.rowsArray[0];
	...
	_suggest: function(records, total, items ) {
		...
		//添加修改 如果包含addButton 的时候 查询数据为空的时候显示添加按钮
		if(items.length==0&&self.options.addButton){
		$('<div class="cg-comboItem cg-menu-item" role="menuitem"><div class="cg-colItem ui-corner-all" tabindex="-1"><div class="cg-DivItem" style="width: 100%; text-align: center;color:red;">没有搜索到符合条件的数据 是否新增？<input type="button" id="addButton" value="新增"/></div></div></div>').appendTo(ul);
		 $("#addButton").click(function(){
		 	//alert(self.element.val());
		 });
		}else{
...
//		ul2.appendTo(ul);
		...
		this._resizeMenu();
		//add by wangjiandong
		var left=parseInt($("body").width())-parseInt(ul.width());
		var top=parseInt($("body").height())-parseInt(ul.height());
		if(this.pos==null){
			ul.position( $.extend({
				of: this.element
		...
		//add by wangjiandong
		/**
		alert(left+"-"+parseInt(ul.css("left")));
		alert(top+"-"+parseInt(ul.css("top")));
		*/
		if(left<parseInt(ul.css("left"))){
			ul.css("left",left+"px");
		}
		if(top>0&&top<parseInt(ul.css("top"))){
			ul.css("top",top+"px");
		}
	...
	_renderHeader: function(ul, colModel ) {
		...
		$(".cg-colHeader-label").bind('click.combogrid',function(event){
			if($(event.target).closest(".combogridOk").length){
				var items=self.options.items;
				var selectedItems=[];
				var count=0;
				$(".combogridCheckbox").each(function(index,box){
					var self=$(this);
					if(self.is(":checked")){
						selectedItems[count++]=items[index];
					}
				});
				self.options.check( event,{items:selectedItems},self.element);
				return;
			}
			self.options.sord="";
			self.cssCol = "";
	...
	_renderLabel: function (label){
		if(this.options.i18n){
			...
			var self = this;
			label=$.isFunction(label)?label(self.element):label
			return label;
		}
	},
    ...
    var colItem;
      ...
      colItem=item[col.columnName];
		...
		//format
		// example 'format':'<img src="rs/rs/img/default.png">{nameCh}'
		//.cg-comboItem{height:50px;}
		//.cg-DivItem{height:50px;}
		if(col.format){
			var ref=/\{\w+\}/gi;
			colItem=col.format;
			var arrMactches = colItem.match(ref);
		  for(var i=0;i < arrMactches.length ; i++){
	        var key=arrMactches[i];
			  colItem=colItem.replace(key,item[key.substring(1,key.length-1)]);
	       }
	      // 复杂的显示可以通过function 返回数值 这个比格式化更灵活
		}
		 if(col.preProcess){
			colItem=col.preProcess(item,{"index":row+1,"pageNo":pageNo,"pageSize":pageSize});
		}
...
// Select page
	...
	_renderPagerPage: function (label, page, total){
		...
		//	return 'Page <input type="text" size="1" class="' +self.element.attr('id')+ ' currentPage" value="' + page + '"></input> of ' + total;
			return '  <input type="text" size="1" readonly="readonly" class="' +self.element.attr('id')+ ' currentPage" value="' + page + '"></input> / ' + total;
		}
	},
	_renderPagerView: function (label, initRecord, lastRecord, records){
		...
		//	return "View " + initRecord + " - " + lastRecord + " of " + records;
			return initRecord+"-"+lastRecord+"/"+records;
		}
	},
	...
	_create: function() {
		...
		this.element
			...
			.click(function( event ) {
				...
				// temporary
				var item=$( event.target ).closest( ".cg-colItem" );
				var checkbox=$(".combogridCheckbox",item);
