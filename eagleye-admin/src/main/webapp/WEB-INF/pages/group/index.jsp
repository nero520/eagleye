...
$(document).ready(function() {
	var url = "<%=basePath%>group/querySimple.html";
	...
	oTable = $('#example').dataTable( {
		...
		"aoColumns":[
			...
			{"mDataProp":"groupName","sTitle":"用户组名称","sClass":"center"},
			{"mDataProp":"createTime","sTitle":"创建时间","sClass":"center"},
			{"mDataProp":"updateTime","sTitle":"最后修改时间","sClass":"center"},
			{
				"mDataProp":null,
				"bSortable":false,
				"sTitle":"成员预警管理<span id='detail_show' class='DataTables_sort_icon css_right ui-icon ui-icon-triangle-1-s'></span>",
				"sClass":"center",
				"fnRender":function(o){
					return "<input type='button' value='成员预警管理' alt='成员预警管理'  title='成员预警管理' class='usergrouplist' groupName='"+o.aData.groupName+"' groupId='"+o.aData.id+"'/>";
				}
			},
			{
				"mDataProp":null,
...
var deleteUrl="<%=basePath%>group/destroy/{id}.html";
var editUrl="<%=basePath%>group/{id}/edit.html";
var addUrl="<%=basePath%>group/add.html";
var detailUrl="<%=basePath%>group/show/{id}.html";

var userGroupUrl="<%=basePath%>usergroup/{groupId}/index.html";



$(function(){
	$(".add").click(function(){
		common.frame(addUrl, '新增用户组','690','290');
		//$("#example").css("width","1098");
	});
	
	$(".edit").live("click",function(){
		var id=$(this).attr("editId");
		common.frame(editUrl.replace(/{id}/ig,id), '修改','690','290');
	});
	
	$(".detail").live("click",function(){
		var id=$(this).attr("detailId");
		common.frame(detailUrl.replace(/{id}/ig,id),"详情",'690','240');
	});
	...
	//获取该用户下所有成员
	$(".usergrouplist").live("click",function(){
		var groupId = $(this).attr("groupId");
		var groupName = "用户组:" + $(this).attr("groupName");
		common.openWin(userGroupUrl.replace(/{groupId}/ig,groupId), groupName ,1);
	});
